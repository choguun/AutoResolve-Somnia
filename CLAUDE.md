...
### Version history (v8–v53; v15 live, v19+v40+v45 contract pending deploy)
...
`scripts/relayer.mjs:RELAYER_VERSION`, `scripts/relayer-smoke.sh` (grep +
echo on the same version string), `app/proof/page.tsx:196` Tooltip
content, and the public docs in `README.md` / `DEPLOYED.md` /
`PITCH_DECK.md`. The v48 polish cycle shipped without honoring this;
v51 restored it. If a future audit cycle catches a vN surface drift, fix
it as MEDIUM and call out the cycle in the changelog entry.

**Bare-catch attribution (v49 L1+L2)**: any new `catch {}` (server or
client) must include a `console.warn` AND a comment citing the audit
cycle that established the convention. Pre-existing bare catches in
`app/api/receipt/[requestId]/route.ts:90,120,158`,
`app/api/receipt/by-tx/[hash]/route.ts:105`,
`hooks/useAgentReceipt.ts:54`, `useMarketCreatedByRequestId.ts:53,66`,
`useGenerationFailures.ts:55,67`, `useRpcHealth.ts:75`,
`components/receipts/AgentReceiptViewer.tsx:149`,
`components/market/ResolutionPanel.tsx:85`, `lib-web/agents.ts:195,224`
already carry attribution — do not remove it.

**useQueryClient import (v50 ship-bug catch)**: `useQueryClient` is
exported from `@tanstack/react-query`, not `wagmi`. v48 L2 added the
import in `components/markets/GenerateMarketForm.tsx` from `wagmi` —
`pnpm build` caught it; `pnpm lint` and `pnpm relayer:smoke` did not.
All 7 consumers in the codebase import from `@tanstack/react-query`.

**ABI lineage**: v19 is the Foundry-tested ABI with 113 tests (104
pre-v40 + 7 v40 `getUserMarkets` + 1 v45 `agentManifest()`, all listed in
`test/AutonomousPredictionMarket.t.sol`). v40 is a contract-only delta
on top of v19 (adds `getUserMarkets(address) → uint256[]` and the
`userMarketIds[user]` / `_userMarketIndex[user][marketId]` storage
pair; 12→14 storage slots; the 3 pre-existing `Inference*Cache` tests
were updated for the slot shift). v45 bumps the on-chain
`agentManifest()` string v19→v40 to document the v40 view. v22–v53 are
frontend/relayer/manifest/tooling only — same compiled bytecode as the
cycle that added the last ABI change.
