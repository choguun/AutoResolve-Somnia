...
| Current contract (v16 — 3 v15-audit HIGH + 3 MEDIUM + 2 LOW closed (MAX_DURATION upper bound + deploy.sh prefund 1→2 STT + persistent parse-failure LRU + retry-from-cache on inference underfunding + by-tx endpoint wired into GenerateMarketForm + receipt proxy secondary retry + generationRequestedAt cleanup invariant + AgentReceiptViewer keyed on requestId)) | [0x764D…2022b](https://shannon-explorer.somnia.network/address/0x764Dc86246D242382c7619Fc715d0E3A64B2022b) |
...
```text
AutonomousPredictionMarket v16 (fully autonomous — creation + resolution + auto-retry relayer + stuck-request recovery + stuck-generation recovery + output cap + 3 v11-audit + 5 v12-audit + 9 v13-audit + 9 v14-audit + 8 v15-audit gap closures; v16 closes 3 v15-audit HIGH + 3 MEDIUM + 2 LOW: MAX_DURATION upper bound on createMarket, deploy.sh prefund 1→2 STT, persistent parse-failure LRU, retry-from-cache on inference underfunding, by-tx endpoint wired into GenerateMarketForm, receipt proxy secondary retry, generationRequestedAt cleanup invariant, AgentReceiptViewer keyed on requestId)
...
Latest v16 deployment — closes the 8 issues surfaced by a fresh audit of v15 (3 HIGH, 3 MEDIUM, 2 LOW). The three HIGHs were: (H1) `deploy.sh` prefunded 1 STT, but v15's resolution pipeline (parse + inference) plus an underfunded-inference retry could drain the contract in a single missed-block burst — bumped to 2 STT; (H2) `createMarket` enforced `MIN_DURATION` but not `MAX_DURATION`, so a creator could mint a market with endTime decades in the future and `requestResolution` (gated on `block.timestamp >= endTime`) would be permanently unable to resolve it — v16 adds `MAX_DURATION = 86400` (1 day, keeps markets resolvable inside `STALE_REQUEST_TIMEOUT` + a few retry cycles); (H3) the relayer's parse-failure URL LRU was in-memory only, so a relayer restart (deploy, host reboot, OOM) wiped the cache and the relayer would re-attempt every previously-failed URL — v16 persists the LRU to `state/parse-failure-cache.json` with an atomic-rename write, and drops the v15 "attemptCount > 0" gate so even fresh-after-restart markets get checked.

- Contract: pending deploy — see DEPLOYED.md for the v16 address once `./scripts/deploy.sh` runs against Shannon.
- Test coverage: 95/95 Foundry tests pass locally (87 v15 baseline + 8 v16 new tests — 2 H2, 2 L1, 3 M1, 1 manifest).
- New v16 contract surface: `MAX_DURATION = 86400` constant + `DurationTooLong()` error + `if (durationSeconds > MAX_DURATION) revert DurationTooLong();` check in `createMarket` (H2); `handleGenerationCallback` now clears `generationRequestedAt[requestId]` on every exit (L1 — matches the v15 `parseRequestedAt` cleanup invariant); new `InferenceUnderfunded(uint256 indexed marketId, uint256 indexed parseRequestId, string parseResult)` event emitted by `_resolveWithLLMInference` on the inference-underfunded path (M1); new `mapping(uint256 => string) public marketParseResult` + new `retryInferenceFromCache(uint256 marketId) external payable nonReentrant returns (uint256 requestId)` that consumes the cache and creates only the inference request (M1); `agentManifest()` body bumped v15 → v16.
- New v16 relayer behavior: persistent parse-failure URL LRU (H3 — `state/parse-failure-cache.json` with debounced atomic-rename write + SIGTERM/SIGINT sync flush; the v15 `attemptCount > 0` gate is dropped so all markets get the check, not just ones seen before in this process); new `INFERENCE_UNDERFUNDED_TOPIC` watch + `drainInferenceUnderfundedEvents` loop + `tryRetryInferenceFromCache` (M1 — uses the inference deposit alone, not the full resolution deposit); new log line `[relayer] starting (v16)`.
- New v16 frontend behavior: `GenerateMarketForm` now fetches `/api/receipt/by-tx/[hash]` on tx confirmation instead of doing a local `decodeEventLog` (M3 — single source of truth for tx-hash → requestId decoding, server handles a 404 gracefully); `AgentReceiptViewer` keyed on `requestId` in `app/receipt/[requestId]/page.tsx` so navigating between receipt pages gives each one a fresh `startedAt` and full polling window (L3); `lib-web/agentManifest.ts` adds `version: 'v16'` and enumerates `retryFromCache: 'retryInferenceFromCache(uint256 marketId) payable'`.
- New v16 API surface: receipt proxy at `app/api/receipt/[requestId]/route.ts` retries the primary host on a 5xx before falling through to the alternate host (M4 — cuts the user-visible "platform appears to be down" rate in half on the typical 200ms blip); `app/api/receipt/by-tx/[hash]/route.ts` now wired into `GenerateMarketForm` (M3); `scripts/deploy.sh` prefund bumped 1 → 2 STT (H1); `state/` added to `.gitignore` for the persistent LRU file.
- v16 is the new live target; v15 is now historical.

Previous v15 deployment (now historical) — closed the 8 issues surfaced by a fresh audit of v14 (1 HIGH, 6 MEDIUM, 1 LOW). The most important fix was **H1**: v14's `handleInferenceCallback` left `parseRequestedAt` set to the original parse timestamp in all three rollback paths (over-long output, invalid YES/NO, non-success status), which misled `getAgentMarketContext` readers — they saw an Open market with `parseRequestedAt != 0`, indistinguishable from a market mid-parse. v15 added `parseRequestedAt = 0` to all three rollback branches and shipped regression tests for each. v16 inherits all v15 behavior; the address changes because v16 added the MAX_DURATION upper bound + persistent parse-failure LRU + retry-from-cache + generationRequestedAt cleanup + 4 other v15-audit gap closures.
