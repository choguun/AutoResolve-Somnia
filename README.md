...
Latest v19+v40+v45 deployment (live on 2026-06-09 at `0x48556EA096F4abFFB569916a138Ec946B54A85dE`, deploy tx [`0x7b7fec…002f8`](https://shannon-explorer.somnia.network/tx/0x7b7fec571d19237307c4f52a2ef2339b4ed959703c6558b8b66a4fe282e002f8)) — ships the v8-v19 audit hardening sequence + v40's `getUserMarkets(address)` view + v45's on-chain `agentManifest()` string bump in a single bytecode. 113/113 Foundry tests pass locally. The relayer (`scripts/relayer.mjs`, v68) is running on Railway (project `autoresolve-somnia`, service `relayer`, healthcheck passing) and has fully autonomous features: relayer-driven auto-liquidity (v62), periodic partial-seed retry for Somnia state-trie issues (v66+v67), stranded-seed observability (v64+v65+v66), backfill-on-startup (v65), and relayer-driven auto-funding (v68). The relayer's first AI-created market on this contract ("Will Somnia mainnet launch before 2027?", tx `0x454a2c…e56c`) is now in `Open` state with the v62 auto-seed (0.01 STT YES + 0.01 STT NO) and the v65 backfill has ensured the 7 pre-v62 markets (1, 2, 5, 6, 7, 8, 9, 10, 11) are also seeded. The dApp's `StrandedSeedsCard` on `/proof` shows `count: 9, totalStrandedStt: "0.180"`.
...
- v52-v53 frontend + relayer + tooling: see `DEPLOYED.md` for the v22-v53 frontend changelog. The relayer's `tryResolveMarket` uses `getResolutionFundingStatus` to compute the top-up needed for each resolution; `tryRetryInferenceFromCache` uses `getGenerationFundingStatus` to compute the top-up needed for cached inference retries. Both top-ups are capped at `RELAYER_MAX_TOPUP_STT` (default 1 STT) per call.
...
- v56 healthcheck shim: new `scripts/healthcheck.mjs` exposes `/health` and `/stranded-seeds` on `0.0.0.0:3000` for Railway's service-level HTTP healthcheck (the relayer's `getLogs` was previously the only liveness signal, and the relayer could appear "alive but stuck" in pathological cases).
- v60 contract prompt-suffix update: the on-chain `GENERATION_PROMPT_SUFFIX` now teaches the inference agent to honor `[duration=N]` hints in the topic text, so the daily auto-create pattern produces 24h markets instead of 5-min ones (the previous suffix said "Prefer a SHORT duration in [300, 600] seconds" which overrode the operator's hint).
- v62 relayer-driven auto-liquidity: new `drainSeedEvents` watches `MarketCreated` events and places a 0.01 STT YES + 0.01 STT NO bet (configurable via `RELAYER_LIQUIDITY_STT`, default 0 = disabled). On `MarketResolved`, the relayer calls `claimWinnings` to recover the seed. Pure relayer-side change, no contract bytecode shift.
- v63 v62-audit cleanup: stranded-seed observability (logs the locked STT when a seeded market's URL is in the parse-failure LRU and detects LRU eviction so the seed can be recovered), dynamic MIN_BET (reads from the contract on startup instead of a hardcoded 0.001 STT), partial-seed completion (avoids double-betting on a YES+NO pair when a prior attempt placed one side before failing), and the env-toggle foot-gun fix (the claim block no longer skips when the operator toggles `RELAYER_LIQUIDITY_STT` to '0' mid-flight).
- v64 dApp surface for stranded-seed observability: new `/api/stranded-seeds` API route derives the stranded set from on-chain data (reads `getUserMarkets` + `getMarket` + `getMarketBets` in parallel). New `StrandedSeedsCard` on `/proof` polls the endpoint and renders count + total STT locked + per-market detail.
- v65 backfill-on-startup: new `backfillSeededMarkets` runs once on the first tick after relayer startup, scans `[1, nextMarketId)`, and seeds any Open market where the relayer EOA hasn't already placed the YES+NO seed. Catches markets created before the v62 auto-seed feature was enabled (on the live contract, 7 markets were backfilled). Idempotent.
- v66 periodic partial-seed retry: new `retryPartialSeeds` runs every `RELAYER_RETRY_PARTIAL_SEED_INTERVAL_TICKS` (default 60 = ~30 min) and scans the seeded markets for any market where the relayer EOA lacks both YES+NO bets, re-attempting the missing side. The stranded-seeds API now tags each entry with a `partialSeed` boolean so the dApp can show a "partial" pill on the `StrandedSeedsCard`.
- v67 v66-audit cleanup: stranded-seeds route requires the relayer EOA to have `userYesBets` AND `userNoBets` of EXACTLY 0.01 STT each to count as stranded (filters out test markets where the relayer EOA has bets with arbitrary amounts). `sttStringToWei` precision helper moved from dApp to `lib-web/contract.ts` for shared use. Partial-seed retry now happens on EVERY tick for markets in the `flaggedPartials` Map (not just every 30 min), with a 60-attempt cap to bound the retry budget.
- v68 relayer-driven auto-funding: new `maybeAutoFundContract` runs every tick. Reads `getResolutionFundingStatus` + `getGenerationFundingStatus` in parallel plus the contract's STT balance and the relayer EOA balance. Sends a plain STT transfer via `walletClient.sendTransaction` (the contract's `receive()` function at L982 already accepts this) to top up the contract to `1.5x RELAYER_AUTO_FUND_STT` (default 0 = disabled). Per-refill cap is `min(0.1 * EOA balance, RELAYER_AUTO_FUND_MAX_PER_REFILL_STT default 2 STT)`. No contract bytecode change.
