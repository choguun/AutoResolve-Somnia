...
| Live contract | [`0x48556E…85dE`](https://shannon-explorer.somnia.network/address/0x48556EA096F4abFFB569916a138Ec946B54A85dE) — deployed 2026-06-09, 113/113 Foundry tests. Full changelog in `DEPLOYED.md`. |
| AI-created market → AI-resolved end-to-end (parse) | pending — see relayer log; the agent pipeline is running on the live contract |
| AI-created market → AI-resolved end-to-end (inference) | pending — see relayer log; the agent pipeline is running on the live contract |
| Historical AI-created → AI-resolved proof (parse) | [4254170](https://agents.testnet.somnia.network/receipts/4254170) |
| Historical AI-created → AI-resolved proof (inference) | [4254291](https://agents.testnet.somnia.network/receipts/4254291) |
| Historical proof (parse) | [2400421](https://agents.testnet.somnia.network/receipts/2400421) |
| Historical proof (inference) | [2400485](https://agents.testnet.somnia.network/receipts/2400485) |
...
During development, we focused on making the project agent-native rather than simply adding AI to a normal dApp. The contract exposes functions for both pipelines — `requestMarketGeneration`, `scanAgentCreatedMarkets`, `getGenerationFundingStatus` for creation, and `scanResolvableMarkets`, `getAgentMarketContext`, `getResolutionFundingStatus`, `agentManifest` for resolution — so an external agent (or a human via `cast`) can drive the full lifecycle without ever touching the frontend. The `/proof` page includes a live "Agent Command Center" that calls these functions against the deployed contract and shows the autonomous call path in real time.

The project is deployed and has a completed end-to-end proof run on a prior contract: an AI agent created market #3 ("Is the capital of France Paris?" with `https://en.wikipedia.org/wiki/Paris` as the source), the same two-stage resolution pipeline ran and produced parse receipt `4254170` and inference receipt `4254291`, the market resolved `YES`, and the on-chain payout is claimable. An earlier contract holds a separate historical proof (parse `2400421`, inference `2400485`, claim tx `0x888327…2380`) that predates the creation feature.
...
### Relayer-driven auto-liquidity

Freshly-created markets can otherwise sit with `yesTotal=0, noTotal=0` for their entire lifetime, so the MarketCard and BetPanel show "0 STT" totals and 50/50 implied odds from the start. The relayer closes that gap: when `RELAYER_LIQUIDITY_STT > 0` (default `0` = disabled for first-deploy safety), the relayer EOA places a small YES+NO seed bet of that amount on every newly-created market and auto-claims the winnings on `MarketResolved`. The seed lands in `marketBets[id]` like any other bet, so the UI just sees the bumped `yesTotal`/`noTotal` — no new component, no new copy. The per-tick cap `RELAYER_SEED_MAX_PER_TICK=5` bounds the burst case (a fresh contract redeploy with 50 historical markets) so the relayer EOA can't blow its own STT balance in a single 30s window. Future work will introduce a real on-chain AMM LP (`addLiquidity`/`removeLiquidity`) — the contract surface is reserved in the manifest but not implemented yet.
...
```text
AutonomousPredictionMarket (live on-chain at 0x48556EA096F4abFFB569916a138Ec946B54A85dE;
  deploy tx 0x7b7fec…002f8 on 2026-06-09;
  113/113 Foundry tests pass)
See DEPLOYED.md for the full changelog.
```
...
| `getUserMarkets(address user) → uint256[]` | Enumerate the markets a user has bet on, in the order they were first bet on. Returns the O(K) position list the My Bets tab reads to replace an O(N) "load every market page and check each" with a single targeted read. After a claim, the market id stays in the array and the frontend reads `userYesBets` / `userNoBets` (which the contract zeroes on `claimWinnings`) to distinguish active positions from history. |
...
Live on Shannon Testnet at [`0x48556EA096F4abFFB569916a138Ec946B54A85dE`](https://shannon-explorer.somnia.network/address/0x48556EA096F4abFFB569916a138Ec946B54A85dE). 113/113 Foundry tests pass locally (`forge test -vv`).

Completed E2E run — AI agent created the market, AI agents resolved it. Market #3 is the artifact — `creator = 0x0000…A1` (the `AGENT_CREATOR_SENTINEL`):
...
Historical E2E proof on an earlier contract (predates the creation feature):
...
4. Open the completed AI-created → AI-resolved receipts ([`4254170`](https://agents.testnet.somnia.network/receipts/4254170), [`4254291`](https://agents.testnet.somnia.network/receipts/4254291)) and the historical proof ([`2400421`](https://agents.testnet.somnia.network/receipts/2400421), [`2400485`](https://agents.testnet.somnia.network/receipts/2400485)).
...
8. **Demonstrate the relayer's autonomous creation (closes the last "human in the loop" gap).** Show `scripts/topics.txt` and run `./scripts/auto-generate.sh` (or start `scripts/relayer.mjs` with `GENERATION_TOPICS_FILE` set). The relayer's `drainTopicFeed` reads the topic list on every tick and submits `requestMarketGeneration` for any topic not already in `state/submitted-topics.<eoa>.json`. New AI-created markets appear on `/` within a few minutes, each with the "Created by AI" badge and `creator = 0x…A1`.

If live agent execution takes longer than the pitch slot, use receipts `4254170` and `4254291`, or the historical proof `2400421` and `2400485`.
...
`pnpm lint`, `pnpm build`, and `forge test` never execute `scripts/relayer.mjs`, so a relayer runtime crash could ship silently. `pnpm relayer:smoke` forks the relayer with a populated env and asserts the process is still alive after 1.5s — it's the missing piece in the verification triangle and must stay green before any merge that touches `scripts/relayer.mjs`. See `scripts/relayer-smoke.sh` for details.
...
- `getUserMarkets` empty / returns betted / no duplicates on rebet / isolates users / both yes and no / ignores non-betted markets / after-claim-winnings.
...
The deployed contract ran an end-to-end proof where the **AI agent created the market and the AI agents resolved it**. Market #3 is the artifact — `creator = 0x0000…A1` (the `AGENT_CREATOR_SENTINEL`):
...
For the earlier E2E proof (with a recorded claim tx), receipts `2400421` + `2400485` + claim tx [`0x888327…2380`](https://shannon-explorer.somnia.network/tx/0x8883273b0bb83dbb7f2cb489b7a5b54b9a7591afeaee58bd472e7fb5b57c2380) remain valid.
...
# O(K) My Bets enumeration (replaces O(N) "load every market and check")
cast call 0x48556EA096F4abFFB569916a138Ec946B54A85dE \
