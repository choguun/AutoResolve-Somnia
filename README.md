...
| Live frontend (v58 — 4 active markets (1h/6h/24h/24h durations) seeded on the v19+v40+v45 contract; v57 LiveCountdown (1Hz client tick on MarketCard + MarketHeader, color shifts cyan → amber → pulsing amber as the resolution window closes); v56 (H0) Daily Resolution Demo on /proof (one AI-created market per day, 24h duration, /api/daily-topic endpoint, scripts/daily-topics.txt operator-curated feed); v55 M1 post-deploy public-doc sweep (v15 → v45 live); v57 (H0) bumped manifest v56 → v57 to advertise LiveCountdown; v58 (H0) renamed home-page "Resolved" tab to "Ended" (time-based filter covers Resolved markets + markets stuck in Open that have hit endTime) + bumped manifest v57 → v58 + seeded 4 active markets so the Active tab isn't empty. `useAgentReceipt` 404 polling cap; SSR contract version; v7 E2E proof section on `/proof`) | [autoresolve-somnia.vercel.app](https://autoresolve-somnia.vercel.app) |
