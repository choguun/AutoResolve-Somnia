...
| Current contract (v17 — 2 v16-audit HIGH + 2 MEDIUM + 2 LOW closed (stale-cache race fix on requestResolution + AgentMarketContext.parseResultCached + receipt proxy contract address + receipt proxy network-error fallback + per-instance parse-failure LRU + pre-check marketParseResult before retryFromCache)) | [0x764D…2022b](https://shannon-explorer.somnia.network/address/0x764Dc86246D242382c7619Fc715d0E3A64B2022b) |
...
```text
AutonomousPredictionMarket v17 (fully autonomous — creation + resolution + auto-retry relayer + stuck-request recovery + stuck-generation recovery + output cap + 3 v11-audit + 5 v12-audit + 9 v13-audit + 9 v14-audit + 8 v15-audit + 8 v16-audit gap closures; v17 closes 2 v16-audit HIGH + 2 MEDIUM + 2 LOW: stale-cache race fix on requestResolution, AgentMarketContext.parseResultCached, receipt proxy contract address, receipt proxy network-error fallback, per-instance parse-failure LRU, pre-check marketParseResult before retryFromCache)
