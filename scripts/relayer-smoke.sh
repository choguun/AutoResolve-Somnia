...
if kill -0 "$PID" 2>/dev/null; then
  ...
  if grep -q "starting (v57)" /tmp/relayer-smoke.log; then
    echo "[relayer-smoke] OK: v57 startup line printed"
  else
    echo "[relayer-smoke] WARN: v57 startup line missing — relayer may be running an older version"
