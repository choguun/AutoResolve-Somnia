...
if kill -0 "$PID" 2>/dev/null; then
  ...
  if grep -q "starting (v67)" /tmp/relayer-smoke.log; then
    echo "[relayer-smoke] OK: v67 startup line printed"
  else
    echo "[relayer-smoke] WARN: v67 startup line missing — relayer may be running an older version"
