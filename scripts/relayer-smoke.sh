...
if kill -0 "$PID" 2>/dev/null; then
  ...
  if grep -q "starting (v68)" /tmp/relayer-smoke.log; then
    echo "[relayer-smoke] OK: v68 startup line printed"
  else
    echo "[relayer-smoke] WARN: v68 startup line missing — relayer may be running an older version"
